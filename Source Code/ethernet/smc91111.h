/*------------------------------------------------------------------------
 ...
 . 04/74		Ynag,Keun-Bae Modified for use with EMPOS_X255
 .
 ---------------------------------------------------------------------------*/
...
//------------ testing coding...  2006-02-28 pungss...
#define SMC_inb(p)	(((SMC_inw((p) & ~3) >> 8) * (p & 0x3)) & 0xff)
...
#define SMC_GET_MAC_ADDR(addr)						\
	do {								\
		unsigned int __v;					\
		__v = SMC_inw(ADDR0_REG );			\
		addr[0] = __v; addr[1] = __v >> 8;			\
		__v = SMC_inw(ADDR1_REG );			\
		addr[2] = __v; addr[3] = __v >> 8;			\
		__v = SMC_inw(ADDR2_REG );			\
