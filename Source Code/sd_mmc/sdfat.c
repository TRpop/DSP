...
////////////////////////////////////////////////////////////
// MASTERBOOT_RECORD에서 사용하는 MBR(master boot record) 갯수
#define PARTITION_NUMBER 4
// PARTITION_TABLE에서 사용하는 BootIndicator
#define PARTITION_NORMAL 0x00
#define PARTITION_SYSTEM 0x80
// PARTITION_TABLE에서 사용하는 SystemId
#define PARTITION_EMPTY  0x00
#define PARTITION_FAT12  0x04 // 1
#define PARTITION_FAT16  0x06 //4
...
typedef struct _PARTITION_TABLE {
    UINT8  BootIndicator;            // 브트 섹터 표시(0x80)
    UINT8  StartingHead;             // 파티션에서 사용하는 스타팅 헤드
    UINT16 StartingSector:6;         // 파티션에서 사용하는 섹터(6 비트)
    UINT16 StartingCylinder:10;      // 파티션에서 사용하는 실린더(10 비트)
    UINT8  SystemId;                 // 파티션 타입
    UINT8  EndingHead;               // 끝 헤드
    UINT16 EndingSector:6;           // 끝 섹터(6 비트)
    UINT16 EndingCylinder:10;        // 끝 실린더(10 비트)
    UINT32 RelativeSector;           // 브트 섹터 
    UINT32 TotalSector;              // 파티션에 할당된 섹터 수
}PARTITION_TABLE, *PPARTITION_TABLE;
//}PARTITION_TABLE;

// 마스터 브트 섹터
#define MASTERBOOT_CODELEN   446      // 코드 크기
#define MASTERBOOT_SIGNATURE 0xAA55   // 서명
...
typedef struct _MASTERBOOT_RECORD{
    ...
    PARTITION_TABLE Partition[PARTITION_NUMBER];    // 파티션 테이블
    UINT16          Signature;                      // 서명
...
typedef struct _RAWPARTITION{
    ...
    UINT16 SectorSize;                         // 섹터 크기(바이트) 
    UINT8  SectorPerCluster;                   // 섹터/클러스트 
    UINT16 ReservedSector;                     // 예약된 섹터
    UINT8  NumberOfFAT;                        // fat 개수(2개)
    UINT16 NumberOfRootEntry;                  // 루트 디렉토리 개수
    UINT16 TotalSector;                        // 총 섹터 수
    UINT8  MediumId;                           // 기억 매체(하드 디스크 : 0xF8)
    UINT16 SectorPerFAT;                       // 섹터/FAT
    UINT16 SectorPerTrack;                     // 섹터/트랙
    UINT16 NumberOfSide;                       // 디스크 드라이버의 헤드 번호  
    UINT32 NumberOfHiddenSector;               // 숨겨진 섹터
    UINT32 TotalSector2;                       // 총 섹터 수(TotalSector=0일 때 사용)
    union{
//	     __unaligned		
            struct{                    // FAT12/FAT16인 경우
            UINT8  PhysicalDiskNumber;         // 드라이브 ID(0x80 : 하드 디스크)
            UINT8  Reserved;                   // 예약
            UINT8  EFDCSignature;              // 확장된 서명 
            UINT32 VolumeSerialNumber;         // 볼륨 시리얼 번호 
            UINT8  VolumeLabel[RAWPARTITION_VOLUMELABLELEN];  // 볼륨/파티션 이름
            UINT8  FileSystemType[RAWPARTITION_FILESYSTEMLEN]; // FAT 타입
            };
...
//        __unaligned 
    ...
    UINT8  Reserved2[RAWPARTITION_RESERVED2LEN];    // 실행 가능한 브트 코드
    UINT16 Signature;                               // 서명
...
//////////////////////////////////////////////
// 디렉토리 섹터에서 사용하는 상수
#define DIRENTRY_NAMELEN      8  // 파일 이름은 8문자까지
#define DIRENTRY_EXTENSIONLEN 3  // 확장자 길이

#define DIRENTRY_NAMELEN1     5  // 이름이 8문자를 넘은 경우
...
// 파일 이름에서 사용하는 특별한 문자
#define DIRENTRY_ENDCHAR   ' '
#define DIRENTRY_DELETED   0xE5  // 파일이 지워진 경우
#define DIRENTRY_NEVERUSED 0x00
#define DIRENTRY_KANJI     0x05
#define DIRENTRY_PADDING   0xFFFF
// 속성 상수
#define DIRENTRY_READONLY    0x01
...
// 시간 계산 매크로
#define DIRENTRY_HOUR(Time)   ((Time)/2048)
...
typedef union _RAWDIRENTRY{
    	struct{                 // 파일 이름이 8문자이내 인 경우
        UINT8  Name[DIRENTRY_NAMELEN];            // 파일 이름
        UINT8  Extension[DIRENTRY_EXTENSIONLEN];  // 확장자
        UINT8  Attribute;                         // 속성 
        UINT8  Reserved;    
        UINT8  CreatedTimeTenth;                  // 만든 시간
        UINT16 CreatedTime;
        UINT16 CreatedDate;
        UINT16 AccessedDate;
        UINT16 StartingClusterHi;                 // 데이터 위치 상위 값
        UINT16 ModifiedTime;                      // 변경한 시간
        UINT16 ModifiedDate;
        UINT16 StartingCluster;                   // 데이터 위치
        UINT32 FileLength;                        // 파일 크기
    	};
    	struct{                  // 파일 이름이 8문자를 넘는 경우
        UINT8  Ordinal;
        UINT16 Name1[DIRENTRY_NAMELEN1]; // 이름의 첫번째 
        UINT8  Reserved1;
        UINT8  Type;                     // 항상 제로
        UINT8  CheckSum;                 // 체크 섬
        UINT16 Name2[DIRENTRY_NAMELEN2]; // 이름의 두번째 
        UINT16 Reserved2;
        UINT16 Name3[DIRENTRY_NAMELEN3]; // 이름의 세번째 
    	};
}RAWDIRENTRY, *PRAWDIRENTRY;
//} RAWDIRENTRY;


//#pragma pack(pop)                        // 바이트 정렬 끝

////////////////////////////////////////////////
// 파일 시스템 구조체에서 사용하는 표현
...
static BOOL GetDirEntryEx(PFILEINFO pFileInfo, PDIRENTRY pDirEntry){
    ...
    if(j != 0){ // long format
        ...
        for(i = j-1; i >= 0; --i){
            ...
            StrCmp(FileName, drEntry[i].FileName);
        }
...
static BOOL InitPartition(BOOL fBootable, DWORD dwFileSystem, DWORD dwStart, DWORD dwLength)
{
	...
	if(PartitionNumber >= MAX_PARTITION){
		printf("@@@ PartitionNumber >= MAX_PARTITION\n");
		return FALSE;
	}
	...
	if(FATReadDisk(pPart, sizeof(*pPart), dwStart) != sizeof(*pPart)){
		...
		printf("\r\n FATReadDisk Error \n");
		printf("\t SectorSize(%d)\n",pPart->SectorSize);
		printf("\t SectorPerCluster(%d)\n",pPart->SectorPerCluster);
		printf("\t ReservedSector(%d)\n",pPart->ReservedSector);
		printf("\t NumberOfFAT(%d)\n",pPart->NumberOfFAT);		
		printf("\t NumberOfRootEntry(%d)\n",pPart->NumberOfRootEntry);			
		printf("\t NumberOfRootEntry(%d)\n",pPart->NumberOfRootEntry);		
		printf("\t TotalSector(%d)\n",pPart->TotalSector);
		printf("\t TotalSector2(%d)\n",pPart->TotalSector2);				
		printf("\t SectorPerFAT(%d)\n",pPart->SectorPerFAT);
		printf("\t Signature(0x%04X)\n",pPart->Signature);		// 0xAA55
		return FALSE;
	}
...
static BOOL InitMasterBootRecord(DWORD dwStart, DWORD dwLength)
{
	...
	/* Debug */
	MasterBootRecordNumber = 1;
	
    	// we just support 26 MBR
    	if(MasterBootRecordNumber >= MAX_PARTITION){
		printf("ERROR : @@ MasterBootRecordNumber(%d) >= MAX_PARTITION(%d)\n",MasterBootRecordNumber ,MAX_PARTITION);
        	return FALSE;
    	}
	
   	// read first sector
	if(FATReadDisk(pMBR, sizeof(*pMBR), dwStart) != sizeof(*pMBR)){
		printf("ERROR : Sector FATReadDisk pMBR Size %d\n", sizeof(*pMBR));
		return FALSE;
	}

	/* Debug.. */
	//printf("Sector FATReadDisk pMBR Size %d\n", sizeof(*pMBR));
	

/*
	// check signature
	if(pMBR->Signature != MASTERBOOT_SIGNATURE){
		printf("ERROR : Check Signature(0x%x) != MASTERBOOT_SIGNATURE(0x%x) \n",pMBR->Signature ,MASTERBOOT_SIGNATURE);
		return FALSE;
	}
*/
...
//				printf("ERROR : check partition table, MemCmp not zero return \n");
...
/*		
		...
		printf("ERROR : boot indicator(%0x), PARTITION(%0x or %0x)\n", pPart1->BootIndicator,PARTITION_NORMAL,PARTITION_SYSTEM);
            return FALSE;
        }
*/        
        // check header(0-254)
        if(pPart1->StartingHead==0xFF || pPart1->EndingHead==0xFF){
		printf("ERROR : boot header(%0x)\n", pPart1->StartingHead);
            return FALSE;
        }
		
        // check sector(1-63)
        if(pPart1->StartingSector==0 || pPart1->EndingSector==0){
		printf("ERROR : boot StartingSector(%0x)\n", pPart1->StartingSector);
            return FALSE;
        }
		...
		printf("ERROR : boot dwLength(%0x) >= dwLength\n",dwPartStart,dwLength);
		return FALSE;
        }
		
        if(dwPartLength==0 || dwPartStart+dwPartLength>dwLength){
		printf("ERROR : boot dwLength(%0x or 0x00) >= dwLength\n",dwPartStart+dwPartLength,dwLength);
            	return FALSE;
        }
        ...
        for(j = i+1; j < PARTITION_NUMBER; ++j){
            ...
            if(pPart2->RelativeSector>=pPart1->RelativeSector &&
               ...
               printf("ERROR : check start point 1 \n");
                return FALSE;
            }
            // check end point
            if(pPart2->RelativeSector+pPart2->TotalSector>pPart1->RelativeSector &&
               ...
               printf("ERROR : check start point 2 \n");               
               return FALSE;
		}
	}
	}
...
void DumpMasterBootRecord()
{
    ...
    for(dwMBRNo = 0; dwMBRNo < MasterBootRecordNumber; ++dwMBRNo){
        ...
        printf("No.|Boot|Head/Track/Sector--Head/Track/Sector|System|Start Sector|Sector Number\r\n");
        for(dwPartNo = 0; dwPartNo < PARTITION_NUMBER; ++dwPartNo){
            printf("%3d|  %02X|%4u/%5u/%6u  %4u/%5u/%6u|    %02X|%12u|%13u\r\n",
                   dwPartNo,
...
void DumpPartitionBootSector(DWORD dwPartNo){
    // 매개 변수 검사
    if(dwPartNo >= PartitionNumber){
        printf("Partition Number beyond scope\r\n");
        return;
    }
    // 덤프
    printf("\r\n\r\n **** Dump Boot Sector ***\r\n");
...
void DumpFATTable(DWORD dwPartNo, DWORD dwStart, DWORD dwCount)
{
    ...
    if(dwStart >= Partition[dwPartNo].FATEntryNumber){
        printf("Start position beyond FAT entry number\r\n");
        return;
    }
    ...
    dwLine = 32/GetFATEntryBytes(dwPartNo); // 한 라인에 32바이트
    // 덤프
    printf("\r\n **** Dump FAT Table ***\r\n");
    for(dwLoop = 0; dwLoop < dwCount; ++dwLoop){
        if(dwLoop%dwLine == 0){
            printf("%04X  ", dwStart+dwLoop);
        }
        printf("%0*X ", dwBits/4, GetFATEntry(dwPartNo, dwStart+dwLoop));
...
void DumpDirectory(PCSTR pDirName, DWORD dwStart, DWORD dwCount)
{
    ...
    if(dwStart >= dwEntries){
        printf("Start position beyond directory entry number\r\n");
        return;
    }
    ...
    printf("\r\n************ Directory Dump **************\r\n");
    printf("     File Name    Length        Created         Att Sum Ord   Cluster\r\n");
    // 지정된 카운트 만큼 각 디렉토리 처리
	i = 0;
    while(dwCount--){
        // 항목 읽기
		...
		if(drEntry.FileLength != 0)
		{
			...
			printf("%02u. "
				   ...
				   "%02X  %02X  %02X  %08X \r\n",
				i,
				drEntry.FileName, drEntry.FileLength, 
				...
				drEntry.Attribute, drEntry.CheckSum, drEntry.Ordinal, drEntry.StartingCluster
				);
			i++;
		}
    }
...
void DumpDirectoryEx(PCSTR pDirName)
{
    ...
    printf("\r\n********** Extended Directory Dump **************\r\n");
    //       12345678901234567**123*12345678**12345678*1...
    printf("Modified           Att Cluster   Length   FileName\r\n");
    ...
    while(1){
        ...
        // output
        printf("%02u/%02u/%02u-%02u:%02u:%02u  %02X  %08X  %-8u %S\r\n",
